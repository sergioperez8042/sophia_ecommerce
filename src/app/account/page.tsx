...
const STATUS_COLORS: Record<OrderStatus, string> = {
  pending: 'bg-[#F5F1E8] text-[#2E4A3A] dark:bg-[#C9A96E]/15 dark:text-[#C9A96E]',
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6">
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center gap-3">
              <Link href="/catalog" className={`p-2 rounded-xl transition-colors ${isDark ? 'text-[#C9A96E] hover:bg-[#C9A96E]/15' : 'text-[#2E4A3A] hover:bg-[#2E4A3A]/10'}`}>
                <ArrowLeft className="w-5 h-5" />
              </Link>
              <h1 className={`text-lg font-semibold ${isDark ? 'text-[#C9A96E]' : 'text-[#2E4A3A]'}`}>Mi cuenta</h1>
            </div>
            <button
              onClick={handleLogout}
              className={`flex items-center gap-2 px-3 py-2 rounded-xl text-sm transition-colors ${isDark ? 'text-[#C9A96E]/70 hover:text-[#C9A96E] hover:bg-[#C9A96E]/10' : 'text-[#2E4A3A]/75 hover:text-[#2E4A3A] hover:bg-[#2E4A3A]/10'}`}
            >
        ...
        >
          <div className="flex items-center gap-4">
            ...
            <label className="relative cursor-pointer group block w-14 h-14 flex-shrink-0">
              <div className={`w-14 h-14 rounded-full overflow-hidden flex items-center justify-center text-lg font-semibold ${isDark ? 'bg-[#C9A96E]/20 text-[#C9A96E]' : 'bg-[#2E4A3A]/10 text-[#2E4A3A]'}`}>
                {user.avatar ? (
                  <Image
                    src={user.avatar}
                    alt={user.name}
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className={`text-sm truncate ${isDark ? 'text-[#C9A96E]/60' : 'text-[#2E4A3A]/75'}`}>{user.email}</p>
            </div>
          </div>
          <div className="grid grid-cols-3 gap-4 mt-5">
            <div className="text-center">
              <p className={`text-2xl font-semibold ${isDark ? 'text-white' : 'text-[#333]'}`}>{orders.length}</p>
              <p className={`text-xs mt-0.5 ${isDark ? 'text-[#C9A96E]/50' : 'text-[#2E4A3A]/65'}`}>Pedidos</p>
            </div>
            <div className="text-center">
              <p className={`text-2xl font-semibold ${isDark ? 'text-white' : 'text-[#333]'}`}>{reviews.length}</p>
              <p className={`text-xs mt-0.5 ${isDark ? 'text-[#C9A96E]/50' : 'text-[#2E4A3A]/65'}`}>Reseñas</p>
            </div>
            <div className="text-center">
              <p className={`text-2xl font-semibold ${isDark ? 'text-white' : 'text-[#333]'}`}>{formatPrice(totalSpent)}</p>
              <p className={`text-xs mt-0.5 ${isDark ? 'text-[#C9A96E]/50' : 'text-[#2E4A3A]/65'}`}>Total</p>
            </div>
          </div>
        </m.div>
            ...
            <button
              ...
              className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-lg text-sm font-medium transition-all ${
                activeTab === tab.id
                  ...
                  : isDark
                    ? 'text-[#C9A96E]/40 hover:text-[#C9A96E]/60'
            ...
            >
              ...
              {tab.count !== undefined && tab.count > 0 && (
                <span className={`text-[10px] px-1.5 py-0.5 rounded-full ${
                  activeTab === tab.id
                    ? isDark ? 'bg-[#C9A96E]/30' : 'bg-[#2E4A3A]/10'
                    : isDark ? 'bg-[#C9A96E]/10' : 'bg-[#2E4A3A]/12'
                }`}>
                  {tab.count}
                </span>
              )}
          ...
          <AnimatePresence mode="wait">
              ...
              >
                {orders.length === 0 ? (
                  <div className="text-center py-16">
                    <ShoppingBag className={`w-10 h-10 mx-auto mb-3 ${isDark ? 'text-[#C9A96E]/30' : 'text-[#2E4A3A]/20'}`} />
                    <p className={`text-sm ${isDark ? 'text-[#C9A96E]/50' : 'text-[#2E4A3A]/65'}`}>Aún no tienes pedidos</p>
                    <Link
                      href="/catalog"
                      className={`inline-flex items-center gap-2 mt-4 px-4 py-2 rounded-xl text-sm font-medium transition-colors ${isDark ? 'bg-[#C9A96E]/20 text-[#C9A96E] hover:bg-[#C9A96E]/30' : 'bg-[#2E4A3A] text-white hover:bg-[#26402F]'}`}
                    >
                      Explorar catálogo
                    </Link>
                  </div>
                      ...
                      >
                        <div className="flex items-start justify-between mb-3">
                          <div>
                            ...
                            <p className={`text-xs mt-0.5 ${isDark ? 'text-[#C9A96E]/50' : 'text-[#2E4A3A]/65'}`}>
                        ...
                        <div className={`space-y-2 ${isDark ? 'divide-[#C9A96E]/10' : 'divide-[#2E4A3A]/15'} divide-y`}>
                          {order.items.map((item, i) => (
                            <div key={i} className={`flex items-center justify-between ${i > 0 ? 'pt-2' : ''}`}>
                              <div className="flex-1 min-w-0">
                                ...
                                <p className={`text-xs ${isDark ? 'text-[#C9A96E]/40' : 'text-[#2E4A3A]/65'}`}>x{item.quantity}</p>
                              </div>
                              <p className={`text-sm font-medium ml-4 ${isDark ? 'text-[#C9A96E]' : 'text-[#2E4A3A]'}`}>
                                {formatPrice(item.price * item.quantity)}
                              </p>
                            </div>
                          ))}
                        </div>
                        <div className={`flex justify-between items-center mt-3 pt-3 border-t ${isDark ? 'border-[#C9A96E]/10' : 'border-[#2E4A3A]/15'}`}>
                          <p className={`text-xs ${isDark ? 'text-[#C9A96E]/40' : 'text-[#2E4A3A]/65'}`}>
            ...
            {activeTab === 'reviews' && (
              ...
              >
                {reviews.length === 0 ? (
                  <div className="text-center py-16">
                    <MessageSquare className={`w-10 h-10 mx-auto mb-3 ${isDark ? 'text-[#C9A96E]/30' : 'text-[#2E4A3A]/20'}`} />
                    <p className={`text-sm ${isDark ? 'text-[#C9A96E]/50' : 'text-[#2E4A3A]/65'}`}>No has escrito reseñas aún</p>
                  </div>
                ) : (
                  <div className="space-y-3">
                    {reviews.map((review) => (
                      ...
                      >
                        <div className="flex items-start justify-between mb-2">
                          <Link
                            ...
                            className={`text-sm font-medium hover:underline ${isDark ? 'text-[#C9A96E]' : 'text-[#2E4A3A]'}`}
                          >
                            {getProductName(review.productId)}
                          </Link>
                          <span className={`text-xs ${isDark ? 'text-[#C9A96E]/40' : 'text-[#2E4A3A]/65'}`}>
                ...
                <div className={`rounded-xl p-5 ${isDark ? 'bg-[#1C2E23] border border-[#C9A96E]/10' : 'bg-white border border-[#2E4A3A]/10'}`}>
                  <div className="flex items-center justify-between mb-5">
                    ...
                    {!editing ? (
                      <button
                        ...
                        className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${isDark ? 'text-[#C9A96E] hover:bg-[#C9A96E]/15' : 'text-[#2E4A3A] hover:bg-[#2E4A3A]/10'}`}
                      >
                      ...
                      <div className="flex gap-2">
                        <button
                          ...
                          className={`p-1.5 rounded-lg transition-colors ${isDark ? 'text-[#C9A96E]/50 hover:text-[#C9A96E] hover:bg-[#C9A96E]/10' : 'text-[#2E4A3A]/65 hover:text-[#2E4A3A] hover:bg-[#2E4A3A]/10'}`}
                        >
                          <X className="w-4 h-4" />
                        </button>
                        <button
                          ...
                          disabled={saving}
                          className={`p-1.5 rounded-lg transition-colors ${isDark ? 'text-[#C9A96E] bg-[#C9A96E]/20 hover:bg-[#C9A96E]/30' : 'text-white bg-[#2E4A3A] hover:bg-[#26402F]'}`}
                        >
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex items-start gap-3">
                      <User className={`w-4 h-4 mt-1 flex-shrink-0 ${isDark ? 'text-[#C9A96E]/40' : 'text-[#2E4A3A]/65'}`} />
                      <div className="flex-1">
                        <p className={`text-xs mb-1 ${isDark ? 'text-[#C9A96E]/40' : 'text-[#2E4A3A]/65'}`}>Nombre</p>
                        {editing ? (
                          <input
                            type="text"
                    ...
                    {/* Email (read-only) */}
                    <div className="flex items-start gap-3">
                      <Mail className={`w-4 h-4 mt-1 flex-shrink-0 ${isDark ? 'text-[#C9A96E]/40' : 'text-[#2E4A3A]/65'}`} />
                      <div className="flex-1">
                        <p className={`text-xs mb-1 ${isDark ? 'text-[#C9A96E]/40' : 'text-[#2E4A3A]/65'}`}>Email</p>
                    ...
                    {/* Phone */}
                    <div className="flex items-start gap-3">
                      <Phone className={`w-4 h-4 mt-1 flex-shrink-0 ${isDark ? 'text-[#C9A96E]/40' : 'text-[#2E4A3A]/65'}`} />
                      <div className="flex-1">
                        <p className={`text-xs mb-1 ${isDark ? 'text-[#C9A96E]/40' : 'text-[#2E4A3A]/65'}`}>Teléfono</p>
                        {editing ? (
                          <input
                            type="tel"
                    ...
                    {/* Address */}
                    <div className="flex items-start gap-3">
                      <MapPin className={`w-4 h-4 mt-1 flex-shrink-0 ${isDark ? 'text-[#C9A96E]/40' : 'text-[#2E4A3A]/65'}`} />
                      <div className="flex-1">
                        <p className={`text-xs mb-1 ${isDark ? 'text-[#C9A96E]/40' : 'text-[#2E4A3A]/65'}`}>Dirección</p>
                        {editing ? (
                          <input
                            type="text"
                    ...
                    <div className="flex items-start gap-3">
                      <MapPin className={`w-4 h-4 mt-1 flex-shrink-0 ${isDark ? 'text-[#C9A96E]/40' : 'text-[#2E4A3A]/65'}`} />
                      <div className="flex-1">
                        <p className={`text-xs mb-1 ${isDark ? 'text-[#C9A96E]/40' : 'text-[#2E4A3A]/65'}`}>Ciudad</p>
                        {editing ? (
                          <input
                            type="text"
                  ...
                  {/* Member since */}
                  <div className={`mt-6 pt-4 border-t ${isDark ? 'border-[#C9A96E]/10' : 'border-[#2E4A3A]/15'}`}>
                    <p className={`text-xs ${isDark ? 'text-[#C9A96E]/30' : 'text-[#2E4A3A]/30'}`}>
