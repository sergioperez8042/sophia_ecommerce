...
export default function ProfilePage() {
  ...
  const getProfileColors = () => {
    if (isAdmin) return { gradient: 'from-[#2E4A3A] to-[#2E4529]' };
    if (isManager) return { gradient: 'from-amber-600 to-amber-800' };
    return { gradient: 'from-[#2E4A3A] to-[#26402F]' };
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 pt-20">
      <div className="container mx-auto px-4 py-8">
        ...
        <Card className="mb-8 border-0 shadow-md">
          <CardHeader className="flex flex-row items-center justify-between">
            <CardTitle className="flex items-center gap-2 text-gray-900">
              <User className="w-5 h-5 text-[#2E4A3A]" />
              Información de Contacto
            </CardTitle>
            {!isEditing ? (
              <Button variant="ghost" size="sm" className="text-[#2E4A3A] hover:text-[#26402F] hover:bg-[#2E4A3A]/5" onClick={startEditing}>
              ...
              <div className="flex gap-2">
                ...
                <Button size="sm" className="bg-[#2E4A3A] hover:bg-[#26402F] text-white" onClick={saveProfile} disabled={savingProfile}>
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="flex items-center gap-3 p-4 bg-gray-50 rounded-lg border border-gray-100 hover:shadow-sm transition-shadow">
                <div className="p-2 bg-white rounded-full text-[#2E4A3A] shadow-sm">
                ...
                <div className="flex-1">
                  ...
                  {isEditing ? (
                    <input
                      ...
                      className="w-full px-2 py-1 border border-gray-300 rounded text-sm font-semibold text-gray-900 focus:outline-none focus:border-[#2E4A3A]"
                    />
                  ) : (
        ...
        {/* CTA por rol */}
        {isAdmin && (
          <Card className="bg-gradient-to-r from-[#2E4A3A] to-[#26402F] text-white">
            <CardContent className="pt-6 text-center">
              ...
              <Link href="/admin">
                <Button className="bg-white text-[#2E4A3A] hover:bg-gray-100 font-bold">
                  Ir al Panel Admin
                </Button>
              </Link>
            </CardContent>
        ...
        {(isManager || isClient) && (
          <Card className="bg-gradient-to-r from-[#2E4A3A] to-[#26402F] text-white">
            <CardContent className="pt-6 text-center">
              ...
              <Link href="/">
                <Button className="bg-white text-[#2E4A3A] hover:bg-gray-100">
                  Ver Productos
                </Button>
              </Link>
            </CardContent>
