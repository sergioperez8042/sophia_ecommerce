...
export default function CatalogHeader({ showBackButton = false }: CatalogHeaderProps) {
    ...
    return (
        <>
            <header
                className={`sticky top-0 z-40 backdrop-blur-xl border-b transition-colors duration-300 ${isDark ? 'bg-[#15241B]/95 border-[#C4AC91]/15' : 'bg-white/80 border-[#2E4A3A]/10'}`}
            >
                    ...
                    <div className="flex items-center justify-between gap-2 h-14 sm:h-20">
                        <div className="flex items-center gap-2 sm:gap-3 min-w-0">
                            {showBackButton && (
                                <Link
                                    href="/"
                                    className={`flex items-center justify-center h-10 w-10 rounded-lg transition-colors flex-shrink-0 ${isDark ? 'hover:bg-[#C4AC91]/15' : 'hover:bg-gray-100'}`}
                                    aria-label="Volver al catálogo"
                                >
                                    <ArrowLeft className={`w-5 h-5 ${isDark ? 'text-[#C4AC91]' : 'text-[#2E4A3A]'}`} />
                                </Link>
                            )}
                            <Link href="/" className="flex items-center gap-2 sm:gap-3 min-w-0">
                                ...
                                <span className={`hidden min-[420px]:inline text-sm sm:text-lg font-semibold tracking-tight leading-tight truncate ${isDark ? 'text-[#C4AC91]' : 'text-[#2E4A3A]'}`}>Sophia</span>
                            </Link>
                        </div>
                        ...
                        <div className="flex items-center gap-1 sm:gap-2 flex-shrink-0">
                            ...
                            <button
                                ...
                                className={`flex items-center justify-center sm:justify-start gap-1.5 h-10 w-10 sm:w-auto sm:px-3 sm:py-2 sm:h-auto sm:min-h-[40px] rounded-xl transition-colors sm:max-w-[200px] ${isDark ? 'bg-[#274034] text-[#C4AC91] ring-1 ring-inset ring-[#36473B] hover:bg-[#2E4A3A]' : 'bg-[#2E4A3A]/10 text-[#2E4A3A] hover:bg-[#2E4A3A]/20'}`}
                            ...
                            <button
                                onClick={toggleTheme}
                                className={`flex items-center justify-center h-10 w-10 rounded-xl transition-colors ${isDark ? 'bg-[#274034] text-[#C4AC91] ring-1 ring-inset ring-[#36473B] hover:bg-[#2E4A3A]' : 'bg-[#2E4A3A]/10 text-[#2E4A3A] hover:bg-[#2E4A3A]/20'}`}
                            ...
                            <Link href="/wishlist">
                                <div
                                    className={`relative flex items-center justify-center h-10 w-10 rounded-xl transition-colors ${isDark ? 'bg-[#274034] text-[#C4AC91] ring-1 ring-inset ring-[#36473B] hover:bg-[#2E4A3A]' : 'bg-[#2E4A3A]/10 text-[#2E4A3A] hover:bg-[#2E4A3A]/20'}`}
                                >
                                    <Heart className="w-4 h-4" />
                                    {wishlistCount > 0 && (
                                        <span className="absolute -top-1 -right-1 min-w-[18px] h-[18px] bg-[#C4AC91] text-[#15241B] text-[10px] font-bold rounded-full flex items-center justify-center">
                            ...
                            {isAuthenticated && user ? (
                                <div ref={avatarMenuRef} className="relative">
                                    <button
                                        ...
                                        className={`w-10 h-10 rounded-full flex items-center justify-center text-xs font-semibold overflow-hidden transition-colors ${isDark ? 'bg-[#C4AC91]/20 text-[#C4AC91] hover:bg-[#C4AC91]/30' : 'bg-[#2E4A3A] text-white hover:bg-[#26402F]'}`}
                                    ...
                                    <AnimatePresence>
                                        {isAvatarMenuOpen && (
                                            <m.div
                                                ...
                                                className={`absolute right-0 mt-2 w-56 rounded-xl shadow-lg border overflow-hidden z-50 ${isDark ? 'bg-[#1C2E23] border-[#C4AC91]/15' : 'bg-white border-[#2E4A3A]/10'}`}
                                            >
                                                <div className={`px-4 py-3 border-b ${isDark ? 'border-[#C4AC91]/10' : 'border-[#2E4A3A]/5'}`}>
                                                    <p className={`text-sm font-medium truncate ${isDark ? 'text-white' : 'text-[#333]'}`}>{user.name}</p>
                                                    <p className={`text-xs truncate mt-0.5 ${isDark ? 'text-[#C4AC91]/50' : 'text-[#2E4A3A]/50'}`}>{user.email}</p>
                                                </div>
                                                <div className="py-1">
                                                    <Link
                                                        href="/account"
                                                        onClick={() => setIsAvatarMenuOpen(false)}
                                                        className={`flex items-center gap-3 px-4 py-2.5 text-sm transition-colors ${isDark ? 'text-[#e8e4dc] hover:bg-[#C4AC91]/10' : 'text-[#333] hover:bg-[#2E4A3A]/5'}`}
                                                    >
                                                ...
                                                <div className={`border-t py-1 ${isDark ? 'border-[#C4AC91]/10' : 'border-[#2E4A3A]/5'}`}>
                                                    <button
